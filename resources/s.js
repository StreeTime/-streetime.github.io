...
function research() {
  ...
  if (SecondLetter == "/") {
    switch (FirstLetter) {
      case "4": window.open("https://boards.4chan.org/" + term.slice(0,2) + "catalog"); break;
      case "r": window.open("https://www.reddit.com/r/" + term.slice(0,2)); break;
  ...
  if (SecondLetter == " ") {
    switch (FirstLetter) { //eg : go(root url,connector,extension)
      ...
      case "n": go("https://search.nixos.org/packages?channel=23.05&from=0&size=50&sort=relevance&type=packages&query=","-",""); break;
